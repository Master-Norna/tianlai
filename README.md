...
| 环境 | 最短入口 | 当前边界 |
| --- | --- | --- |
| Windows 10/11 x64 | [Windows 三步上手](#windows-三步上手) | `1.1.0` 完整参考平台 |
...
Windows 10/11 x64 与 64 位 CPython 3.11–3.14 是 `1.1.0` 的参考环境。
...
`1.1.0` 推荐使用下面这一条主链，而不是分别调用早期导入和合奏命令后手工拼接
产物：
...
工作流（作品宪章、分阶段复核、可信 hard failure、例外、受管渲染、修订、回滚与
审计）；MCP 另提供立宪后按需使用的无状态外部宪法条文查询。`1.0.0` 在这条工作流上新增 Claim Lifecycle v1
...
作品宪章先行，它与作品自身材料共同构成 workflow 的根。外部音乐宪法只是立宪后
可按需读取的无状态思想资源；完全不使用它，展开图、逐题复核、推导、证据、验收和
续作仍保持完整。条款不绑定生成、复核、验收或续作。旧 workflow 中任何既存 binding
都只保留“当时引用过什么”的不可变历史来源，不进入当前裁决，也不阻断继续创作；
其中 v0.1 已废弃，不再回查旧条文或映射到 v0.2。
旧客户端应从 activation 删除 `constitution` / `active_clauses`，或分别传 `null` 与
`null`/空数组；需要思想参考时，在立宪后另行调用当前 v0.2 的无状态查询。
...
这些步骤。位置审视同时保留两条路：能从作品既有材料和关系长出的内容，如实追索其
前因后果；没有这层血缘、却对全曲确有必要的内容，可以作为无血缘的全局选择接受。
不得为了保留一个细节伪造因果；若两条路都不成立，留白、静音或删除同样是完整结论。
