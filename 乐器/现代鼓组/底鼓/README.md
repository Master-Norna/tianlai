[中文](README.md) | [English](README.en.md)

# 底鼓(formal)

VCSL 大鼓近距离混合话筒,4 力度层×2RR 作现代底鼓。本目录是 98 项清单 SAM-25 的专用实现,渲染引擎复用
`tianlai/dedicated_sfz.py`,不含通用 SoundFont 静默回退。

## 来源与许可

- 上游:sgossner/VCSL (Versilian Community Sample Library)
- 版本:1.2.2-RC,许可:CC0-1.0
- 逐文件 SHA-256 与统计见 [`资源核验.json`](资源核验.json),复算脚本 [`核验资源.py`](核验资源.py)

## 映射与奏法

- `hit`:`Membranophones/Struck Membranophones/Bass Drum 1.sfz`

默认奏法 `hit`;pitch_mode `fixed`。

## 键位映射

| MIDI 键 | 内容 |
| --- | --- |
| 60 | 近距离大鼓 hit,4 力度层×2RR |

## 音域

见键位映射

## 调音

底鼓为无固定音高膜鸣打击,任意谱面音高都触发同一鼓,不做十二平均律校准(详见 [`音准校准.json`](音准校准.json))

## 试听

固定事件:`examples/底鼓_奏法.events.json`;
渲染 5.50 s,峰值 0.419928,
RMS 0.035118,削波 0;
WAV SHA-256 `095d759f…`。复算脚本 [`核验试听.py`](核验试听.py)。

## 已知限制

来源为音乐会大鼓近讲;无鼓槌拍击面选择。当前绑定版本已通过单音色试听并标为 formal；协奏、完整奏法与实际曲目仍未测试。
