"""渲染风笛固定试听并复算 WAV 指标与 Hash。"""

from pathlib import Path
import sys


ROOT = Path(__file__).resolve().parents[3]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from tianlai.dedicated_candidates import generate_dedicated_audition_verification


def main() -> None:
    here = Path(__file__).resolve().parent
    report = generate_dedicated_audition_verification(
        here / "乐器.json",
        ROOT / "examples" / "风笛_奏法.events.json",
        ROOT / "output" / "风笛_candidate.wav",
        output_path=here / "试听核验.json",
        coverage=[
            "旋律管 E4-A5(64-81)；低音管 G2(43) 与高音管 G3(55) 以独立奏法同时持续",
            "弱/中/强三档力度与长短音",
            "奏法:chanter、drone_low、drone_high",
            "note-off 释放与尾音",
        ],
    )
    print(f"峰值 {report['peak']:.6f},削波 {report['clipped_samples']}:{here / '试听核验.json'}")


if __name__ == "__main__":
    main()
