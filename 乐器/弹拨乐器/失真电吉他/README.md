...
确定性信号链:120 Hz 高通 → 硬削波(pre 14 / post 0.5)→ 4.3 kHz 低通,模拟高增益失真通道。全部参数固化在 `乐器.json` 的
...
固定事件:`examples/失真电吉他_奏法.events.json`;
渲染 10.15 s,峰值 0.420000,
RMS 0.206494,削波 0;
WAV SHA-256 `f7e274f1…`。复算脚本 [`核验试听.py`](核验试听.py)。

## 已知限制

失真由确定性波形整形获得,非失真音箱实录;无哨音/闷音技巧采样。当前绑定版本已通过单音色试听并标为 formal；协奏、完整奏法与实际曲目仍未测试。
