...
- Events use sounding pitch: the sampled sounding range is Ab3–E6 (MIDI 56–88). The written range of the B♭ soprano saxophone is Bb3–F#6 (58–90), converted by `sounding pitch = written pitch - 2 semitones`.
...
- All 66 deduplicated pitched FLAC files read their `riff/smpl` loops; 80 breath-noise and 28 key-noise recordings are also connected.
- Playback tuning follows the upstream `key/pitch_keycenter + tune` exactly. Per-file FFT diagnostics over the loops of all 66 files have a median relative residual of `+0.597 cents` and a maximum absolute difference of `16.960 cents`. Natural vibrato in the upper loops makes single-window peaks fluctuate, so diagnostics do not overwrite the author's tune table.
...
{ "time": 0.0, "type": "note_on", "note_id": 1, "midi_note": 72, "velocity": 0.8 }
{ "time": 1.2, "type": "note_off", "note_id": 1 }
```

`midi_note` is always sounding pitch. The collaboration layer can later read
`written_to_sounding_semitones=-2` to convert written pitch to sounding pitch.

```powershell
.\.venv\Scripts\python.exe 乐器/现代管乐/高音萨克斯/核验资源.py
.\.venv\Scripts\python.exe 乐器/现代管乐/高音萨克斯/校准音准.py
.\.venv\Scripts\python.exe 乐器/现代管乐/高音萨克斯/核验试听.py
```
